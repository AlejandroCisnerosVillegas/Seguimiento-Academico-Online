...
<?php
  ...
  $con = connectTo();
  $query = $con->query("select email from project_02_teacher where email = '".$email."'");
  if($query && $con->affected_rows && $email != $_SESSION['email']) respond("error","exists");
  $query = $con->query("select email from project_02_teacher where uid = '".$teacher_id."'");
  if($query && $con->affected_rows) {
    $q = "update project_02_teacher set ";
    if($name != '') $q .= "name = '".$name."'";
