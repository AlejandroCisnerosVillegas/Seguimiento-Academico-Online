  ...
  class Node {
    ...
    public function retrieveObject($code,$section,$year) {
      ...
      if($con->connect_errno) {
        ...
        $obj = $con->query('select object from project_02_objects where code = "'.$code.'" and section = "'.$section.'" and year = "'.$year.'"');
        if($con->errno) {
          return false;
        } else {
            if($obj->num_rows == 1) {
    ...
    public function retrieveObjecti($class_id,$teacher_uid) {
      ...
      if($con->connect_errno) {
        ...
        $obj = $con->query('select object from project_02_objects where uid = "'.$class_id.'" and teacher_uid = "'.$teacher_uid.'"');
        if($con->errno) {
          return false;
        } else {
            if($obj->num_rows == 1) {
    ...
    public function deleteNode() {
      ...
      if($con->connect_errno) {
        ...
        $year = $this->getYear();
        $obj = $con->query('delete from project_02_objects where teacher_uid = "'.$teacher_uid.'" and code = "'.$code.'" and section = "'.$section.'" and year = "'.$year.'"');
    ...
    public function saveNode() {
      ...
      $section = $this->getSection();
      $year = $this->getYear();
      $obj = $con->query('select object from project_02_objects where teacher_uid = "'.$teacher_uid.'" and code = "'.$code.'" and section = "'.$section.'" and year = "'.$year.'"');
      if($obj->num_rows)
        $obj = $con->query('update project_02_objects set object = "'.$con->real_escape_string(serialize($this)).'" where teacher_uid = "'.$teacher_uid.'" and code = "'.$code.'" and section = "'.$section.'" and year = "'.$year.'"');
      else 
        $obj = $con->query($q= 'insert into `project_02_objects`(`teacher_uid`, `code`, `year`, `section`, `object`) VALUES ("'.$teacher_uid.'","'.$code.'","'.$year.'","'.$section.'","'.$con->real_escape_string(serialize($this)).'")');
        if($con->errno) {
          return false;
        } else {
          return true;							
    ...
    public function saveNodei($class_id) {
      ...
      if($con->connect_errno) {
        ...
        $section = $this->getSection();
        $year = $this->getYear();
        $selectedNode = $con->query('select object from oproject_02_objects where uid = '.$class_id.' and  teacher_uid = '.$teacher_uid);
        if($selectedNode && $con->affected_rows) {
          $obj = $con->query('update project_02_objects set code = "'.$code.'", year = "'.$year.'", section = "'.$section.'", object = "'.$con->real_escape_string(serialize($this)).'" where teacher_uid = "'.$teacher_uid.'" and uid = "'.$class_id.'"');
          if($obj && $con->affected_rows) {
            return true;
          }        
          return false;
    ...
    public function getTeacherName() {
      $con = connectTo();
      $s = $con->query("select name from project_02_teacher where uid = ".$this->getTeacherID());
      $name = $s->fetch_assoc();
      $name = $name['name'];
      return $name;
    }
