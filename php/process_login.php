...
$con = connectTo();
$exists = $con->query("select * from `general`.`project_02_teacher` where email = '$email'");
...
if(verifyPass($pass,$exists['password'])) {
  ...
  $_SESSION['classes'] = 0;
  $classes = $con->query('select uid from `project_02_objects` where teacher_uid = '.$_SESSION['teacher_id']);
  ...
  $con->close();
  respond("error","Correo electrónico o contraseña ingresada son incorrectos");
}
?>